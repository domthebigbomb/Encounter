...
- (void)getDirectionsFrom:(NSString*)origin to:(NSString*)destination
          withTransitType:(NSString*)transitType succeeded:(void(^)(GMDirection *directionResponse))success
                   failed:(void (^)(NSError *error))failure{
    
    NSString *path = [[NSString alloc] initWithFormat:@"%@?sensor=%@&origin=%@&destination=%@&mode=%@",
                      DIRECTION,
                      SENSOR,
                      origin,
                      destination,
